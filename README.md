...
如果团队里由律师确认模板、技术同事负责部署，可以直接按这个清单交接：

| 交接内容 | 说明 |
|---|---|
| 模板 DOCX | 使用正式机构模板，先把真实案件内容删掉。 |
| 占位符清单 | 例如 `{{CASE_NO}}`、`{{CLAIMANT}}`、`{{RESPONDENT}}`，说明每个占位符代表什么。 |
| 替换数据样例 | 使用模拟名称、模拟案号和模拟金额，不要使用真实客户资料。 |
| 格式要求 | 标题、字体、字号、行距、页眉页脚、页码、落款和签名区要求。 |
| 交付标准 | 明确哪些检查必须 PASS，哪些 WARNING 需要律师人工确认。 |
