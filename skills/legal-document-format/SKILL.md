...
description: 面向法律文书格式处理的 Agent Skill，支持内容锁定、DOCX 模板继承和发布档强制 LibreOffice + Poppler 视觉校验。
---
...
- 如果用户要求“和模板一模一样”，优先走 V2 base-replace：复制模板 DOCX 包结构，只替换明确占位符，并运行模板一致性门禁。
...
- `scripts/apply_docx_template.py`：从用户 DOCX 模板复制包结构并替换 `{{KEY}}` 文本占位符。
...
- `scripts/release_smoke.py`：V2 发布 smoke gate，覆盖依赖、模板应用、模板一致性、渲染、强制视觉门禁和测试。
